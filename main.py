...
class MyPlugin(Star):
    ...
    async def character_card(self, event: AstrMessageEvent, uid: str = None, character_index: int = None):
        ...
        if character_index is None:
            try:
                ...
                # 捕获其他未知错误
                logger.error(f"角色列表生成失败 | UID: {uid} | 错误: {str(e)}", exc_info=True)
                yield event.plain_result(f"❌ 生成角色列表时发生错误: {str(e)}")
    ...
    async def custom_t2i_tmpl(self, event: AstrMessageEvent):
        options = {} # 可选择传入渲染选项。
        with open('/Astrbot/data/plugins/astrbot_plugin_enkacard/screen_role_list/269377658_2026418_180516.html', 'r', encoding='utf-8') as f:
            TMPL = f.read()
